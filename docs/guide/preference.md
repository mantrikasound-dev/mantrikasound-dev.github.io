...
把 Mantrika 推荐的双击 / Modifier 配置写入 REAPER 的 Mouse Modifiers。可以**一键全部**，也可以**逐条**开关。

| 控件                          | 作用                                                                                            |
| --------------------------- | --------------------------------------------------------------------------------------------- |
| **Apply All**               | 一键应用全部推荐配置。全部已应用时按钮变灰、文案改为 "All Applied"。                                                 |
| **Restore All**             | 一键还原所有已应用项到原始状态。只要还有任意一项处于已应用，按钮就可点。                                                      |
| **逐条功能开关**（列表）              | 每个逻辑功能一行（带说明）。勾选 = 应用该项；取消 = 还原该项。原始配置按项独立备份，取消时逐项还原。                                      |

- 各功能开关的状态实时反映真实配置：若某项写入失败，开关会自动弹回正确位置并弹窗提示。
