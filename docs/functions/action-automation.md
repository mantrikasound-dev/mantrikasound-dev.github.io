...
典型用途：开 REAPER 就自动打开你常用的几个窗口、套好屏幕布局、启动自动化action等等——开机即就位。
...
| **⟳** 按钮 | 设置 / 更换触发动作（弹动作选择器） |
| **FollowUp 列表**（下方） | 触发后要依次执行的动作，每行带 ▲▼✕（上移/下移/删除） |
| **Add FollowUp** | 往列表末尾加一个后续动作 |

> 没选中任何链时，右栏是空的、`⟳` 和 `Add FollowUp` 按钮是灰的——**先在左栏选一条链**它们才可用。
...
- **多条链各管各的**：可以建多条链，互不干扰；可以给不同的触发动作各配一套后续。
- **链不会套娃**：某条链正在跑它的后续动作时，期间触发到别的链的触发动作**不会再触发**——这是有意为之，防止自动触发自动、绕成死循环。所以别指望"A 触发 B、B 又触发 C"这种连环自动接力。
