...
> **Show Advanced** 勾开时，会按当前 Simple 的两个推子把 5 个 Advanced 参数**算一遍当起点**，之后你可以单独细调。
...
适合：从环境录音里挑出有意思的"事件"（鸟叫、车过、人声片段），或者反过来留下干净的Pad做 ambience。
