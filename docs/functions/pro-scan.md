...
```
┌──────────────────────────────────────────────────────────────────┐
│  [Filter by Name... (AND OR NOT)        ]    ☐ Solo ShowOnly    │  ← 第 1 行
│  [Filter by FX... (AND OR NOT)          ]    ☐ Sort by Name     │  ← 第 2 行
│  [Show Only Selected] [Show All] [Mixer]      ☐ Natural          │  ← 第 3 行
├──────────────────────────────────────────────────────────────────┤
│ REAPER Sel (3) | All: 124 tracks, 580 items | ShowOnly: 5 tracks │  ← 状态栏
├──────────────────────────────────────────────────────────────────┤
│   [ Tracks (124) ]    [ Items (580) ]                            │  ← Tab 行
├──────────────────────────────────────────────────────────────────┤
│  [Folders] [Tracks]                                              │  ← 分组开关（仅无筛选时）
│                                                                  │
│   v  M S FX   ▶ Drums Bus (12 tracks)                            │
│        M S FX     Kick                                           │
│        M S FX     Snare                                          │
│        M S FX     OH L                                           │
│        ...                                                       │
│        M S FX     Click                                          │
│        M S FX     Master Reverb                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
| ☑ **Natural** | 在无筛选时，把顶级 folder 和顶级单轨道**混合在一起**按工程顺序排列，而不是分成两段（详见 §5.4） |
...
| 操作 | 行为 |
|---|---|
| **左键单击轨道名字** | 选中该轨道 + 跳转到该轨道（在 REAPER 里 SetOnlyTrackSelected 并垂直滚动到该轨道） |
...
```
Items (580):     ☐ Only Mirrors
─────────────────────────────────
  M   Footstep_Wood_01
  M   Footstep_Wood_02
  M   Vocal_Lead_Take_03
  ...
```
...
| Mixer 拖动滑得太快 | 默认灵敏度对当前 DPI 偏高 | 按住 Shift 进入精细模式（÷10） |
