...
**Project Manager** 是 Mantrika Tools 的Reaper工程管理工具，把分散在 REAPER 历史记录、磁盘各处的工程文件、自己整理的虚拟分类、收藏等等，统一在一个的窗口里管理。可以快速搜索、打开、整理、备份你的所有 REAPER 工程。
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│ [New Project ▼] [Search ............] [Backup] [Refresh]            [☰ ]    │  ← Toolbar
├──────────────────────┬───────────────────────────────────────────────────────┤
│ Scanned Projects (0) │  Folder Name (5)                    [Open Tab] [📂]  │  ← Breadcrumb
│ History Projects (X) │ ┌──┬────────────┬──────────────┬────────────┬──────┐  │
│ Favorites        (Y) │ │★ │ Name       │ Path         │ Modified   │ Note │  │
│                      │ ├──┼────────────┼──────────────┼────────────┼──────┤  │
│ ▶ Virtual Folders    │ │★ │ MyTrack    │ D:\Music\... │ 2026/05/19 │      │  │
│   ▶ ⬛ Personal      │ │☆ │ Demo_v01   │ E:\Demos\... │ 2026/05/12 │ wip  │  │
│     ⬛ Demos         │ │☆ │[Sub] sub01 │ ...          │ ...        │      │  │
│   ▶ ⬛ Client A      │ │☆ │[X] old.rpp │ ...          │ ...        │      │  │  ← 删除项
│                      │ └──┴────────────┴──────────────┴────────────┴──────┘  │
│ [New][Rename][Del]   │                                                       │
│                      │                                                       │
│ ☐ FX Offline         │                                                       │
...
```
┌─ Backup Projects ───────────────────────────────────┐
│  Target Directory: [______________________] [Browse]│
│  ☑ Create subfolder for each project                │
│                                                     │
│  Projects to backup:                                │
│  ┌─────────────────────────────────────────────┐    │
│  │ Original name 1   →   [backup_name_1______] │    │
│  │ Original name 2   →   [backup_name_2______] │    │
│  │ Original name 3   →   [backup_name_3______] │    │
│  └─────────────────────────────────────────────┘    │
│                                                     │
│            [ Cancel ]   [ Backup ]                  │
└─────────────────────────────────────────────────────┘
...
适合：**初次完整备份**、归档发行版本、给别人发包。

**模式 B：Create subfolder = OFF**

所有工程的媒体都扁平放进同一个 `Media/` 文件夹，差异复制：
...
├── ProjectC.rpp
└── Media/
    ├── voice.wav        ← 同名文件不会被覆盖，自动加 _2、_3 后缀
    ├── drums.wav
    └── ...
```

适合：**多个共享素材库的工程一起备份**——已经在目标 Media 里的文件不会重复复制（差异同步）。

### 9.4 自定义备份名

每行右侧的输入框可以改本次备份的名字：

- 模式 A：决定子文件夹名（同时 `.rpp` 也跟着改名）
- 模式 B：决定输出 `.rpp` 的文件名
...
│  Scan Paths:                                       │
│  ┌──────────────────────────────────────────────┐  │
│  │ D:\Music\Projects        (124 projects)      │  │
│  │ E:\Demos                 (42 projects)       │  │
│  │ F:\Client\ProjectA       (8 projects)        │  │
│  └──────────────────────────────────────────────┘  │
│                                                    │
│  [ + Add Path ]  [ Remove ]  [ Refresh ]           │
│                                                    │
│  [ Cancel ]              [ Confirm & Scan ]        │
└────────────────────────────────────────────────────┘
```
...
│  ☑ Show Note Column                 │
│  ─────────────────────────────────  │
│  ☐ Auto-hide after opening project  │
│  ☐ Hide Excluded Projects           │
│  ─────────────────────────────────  │
│  [ Manage Exclude Paths... ]        │
│  [ Manage Scan Paths... ]           │
│  [ Path Relocate... ]               │
└─────────────────────────────────────┘
```
...
模式 A 下，如果目标位置已经有同名子文件夹，PM 会自动给本次备份追加时间戳（`_bak_MMDD_HHMM`）。所以多次备份同一个工程不会互相覆盖。

### 17.6 模式 B 备份的差异复制

模式 B（不创建子文件夹）下，已经在目标 Media 里的同名文件**不会**被重新复制。但如果你改过这个文件的内容、又想用新版本，请先手动删掉目标里的旧版本。
...
| 模式 B 备份后 Media 里没新文件 | 同名文件已经存在被跳过 | 删除目标里的旧版本再重新备份 |
